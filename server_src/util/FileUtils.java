...
public class FileUtils {
    ...
    public static void appendFileAsStrings(boolean lineBreak, String path, String... data) {
        if (doesFileExist(path)) {
            String[] currentData = readFileAsStrings(path);
            String[] d = new String[currentData.length + data.length];
            int v = 0;
            for (String s : currentData) {
                if (lineBreak && !s.endsWith(getLineBreak())) s += getLineBreak();
                d[v++] = s;
            }
            for (String s : data) {
                if (lineBreak && !s.endsWith(getLineBreak())) s += getLineBreak();
            ...
            if (d[d.length - 1].endsWith(getLineBreak()))
                d[d.length - 1] = d[d.length - 1].substring(0, getIndexLastOccurance(d[d.length - 1], getLineBreak()));
            writeFileAsStrings(lineBreak, path, d);
        } else {
    ...
    /**
     * Prints the given key-value-map to a String or Console.
     *
     ...
     * Scans line by line if applicable.
     *
    ...
    public static String printFile(String path, boolean console) {
        ...
        for (String str : strs) {
            ...
            if (k++ != strs.length && !str.endsWith(getLineBreak())) sb.append(getLineBreak());
        }
        if (console) System.out.println(sb.toString());
        return sb.toString();
    }
    ...
    /**
     * String to separate each byte in the following two functions;
     * should be unique and should not be representative
     * of a number or any other data type or regex call.
     */
    private static final String byteSep = "@";
    ...
    public static String byteArrayToString(Byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            sb.append(bytes[i]);
            if (i + 1 < bytes.length) sb.append(byteSep);
        }
        return sb.toString();
    }
    ...
    public static Byte[] stringToByteArray(String str) {
        String[] strings = str.split(byteSep);
